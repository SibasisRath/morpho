...
The studio (`studio.py` + `studio.html`, dev-only, never deployed) imports `bake()` from `make_face_data.py`, serves the site in an iframe with the in-memory bake at `/site/face-data.js`, and drives live look changes through the site's postMessage config API (`index.html` section 8, also usable from the console as `SWARM.configure({size: 1.5, colorMode: 'rainbow', …})`). Beyond bake knobs and look controls it has: caption text/typography/reveal controls with presets (`configure({text})`), a crop-selection canvas and paint-correction brush (green adds dots, red erases → `--crop`/`--edits` CLI flags), face/object subject mode, 10-step undo/redo, WebM recording (`configure({record:true})` — a compositor redraws background + caption onto the captured canvas inside the frame loop; a WebGL canvas is only readable right after render), and embed export (zip with `window.MORPHO_DEFAULTS` injected; section 8c applies it at boot). "Save to project" writes `face-data.js` + the winning params to `.studio/last-saved-params.json`.
