...
"Morpho" — a static WebGL art site where thousands of GPU-simulated butterflies flock and then settle into a portrait made of light. No build step, no framework, no package.json. The working copy lives in `files/`; `assets/` holds source images and reference material; `files/morpho.zip` is a packaged distribution of the same site (and contains `face-data.js` + `placeholder_face.jpg`, which are not extracted in `files/`).
...
Run the site (from `files/`, after making sure `face-data.js` sits next to `index.html` — extract it from `morpho.zip` if missing):
...
This also writes `face-preview.png` (dot/depth preview) for checking the mask before opening the site. `files/morpho/face-data.orig.js` is a backup of the original full-density (non-stippled) cloud. Optional flags: `--size 1024` (working resolution, helps if the mask misses hair), `--depth depth.png` (external monocular depth map for true relief), `--depth-scale`, `--seed`.
...
- **0. Device tier** — picks butterfly count (72²–128²), bloom on/off, and pixel-ratio cap from cores/mobile detection (see the table in `files/README.md`).
